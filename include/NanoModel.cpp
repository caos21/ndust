...
//
// -------------------------------- Constants ---------------------------------
int NanoModel::read_constants() {
  int err = 0;
  BOOST_LOG_SEV(lg, info) << "Constants parameters: ";
  
  err = read_attrib_hdf5<double>(h5obj, "/Constants", "indens",
                                 cs.indens);

  BOOST_LOG_SEV(lg, info) << "--> Nanoparticle initial density: " << cs.indens;

  
  err = read_attrib_hdf5<double>(h5obj, "/Constants", "qtol",
                                 cs.qtol);

  BOOST_LOG_SEV(lg, info) << "--> Nanoparticle qtol: " << cs.qtol;
